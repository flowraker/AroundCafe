...
//    private final Executor paymentThread;
//
//    @Override
//    @Transactional
...
//
//        List<CompletableFuture<PaymentResponse>> paymentDtoList = paymentList.stream().map(
//                payment -> CompletableFuture.supplyAsync(() -> payment2PaymentResponse(payment), paymentThread)
//        ).collect(Collectors.toList());
//
//        return paymentDtoList.stream().map(CompletableFuture::join).collect(Collectors.toList());
//
//    }
//
//    @Override
...
//        log.info("get payment Response list :" + Thread.currentThread().getName());
...
//
//            log.info(payment.getPaymentNo().toString() + " payment To Response :" + Thread.currentThread().getName());
...
//
//            List<CompletableFuture<OrderItemResponse>> orderItemList = payment.getOrderItem().parallelStream().map(
//                    orderItem -> CompletableFuture.supplyAsync(() -> orderItemToDto(orderItem), paymentThread)).collect(Collectors.toList());
//
//            List<OrderItemResponse> orderItemResponseList = orderItemList.stream().map(CompletableFuture::join).collect(Collectors.toList());
//            paymentResponse.setOrderItems(orderItemResponseList);
//
...
//        log.info(orderItem.getPayment().getPaymentNo().toString() + " orderItem To Dto :" + Thread.currentThread().getName());
//        OrderItemResponse orderItemResponse = OrderItemResponseMapStruct.instance.toDto(orderItem);
//        CafeMenu cafeMenu = menuRepository.findById(orderItem.getCafeMenuNo())
//                .orElseThrow(() -> new ResourceNotFoundException("cafeMenu", "cafeMenuNo", orderItem.getCafeMenuNo()));
...
//            this.payment2PaymentResponse(payment);
//        }
